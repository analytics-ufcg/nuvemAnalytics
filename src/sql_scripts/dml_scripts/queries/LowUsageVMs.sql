select V.vm_name, tabelaCpu.cpu_alloc, tabelaDisk.ios_per_sec, tabelaNet.pkt_per_sec
from (select V.vm_name, V.id_vm, C.cpu_alloc, C.id_time, NTILE(100) OVER(order by C.cpu_alloc) as cpu_p 
	 ...
	 (select V.vm_name, V.id_vm, N.pkt_per_sec, NTILE(100) OVER(order by N.pkt_per_sec) as pkt_p
	  ...
	  where (V.id_vm = N.id_vm)) as tabelaNet, time_dim T, vm_dim V
where tabelaCpu.cpu_p > 90 and tabelaCpu.cpu_alloc < 1000000 and
